...
func (s *PaymentService) invokeProvider(ctx context.Context, order *dbent.PaymentOrder, req CreateOrderRequest, cfg *PaymentConfig, payAmountStr string, payAmount float64, plan *dbent.SubscriptionPlan) (*CreateOrderResponse, error) {
	...
	return &CreateOrderResponse{
		OrderID: order.ID, Amount: order.Amount, PayAmount: payAmount, FeeRate: order.FeeRate,
		Status: OrderStatusPending, PaymentType: req.PaymentType, PayURL: pr.PayURL,
		QRCode: pr.QRCode, ClientSecret: pr.ClientSecret, PayAddress: pr.PayAddress,
		CryptoAmount: pr.CryptoAmount, CryptoCode: pr.CryptoCode, Network: pr.Network,
		ExpiresAt: order.ExpiresAt, PaymentMode: sel.PaymentMode,
	}, nil
}
