...
func TestOpenAIHandleErrorResponse_MissingToolOutputReturnsActionableBadRequest(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)

	svc := &OpenAIGatewayService{}
	respBody := []byte(`{"error":{"message":"No tool output found for tool search call fcbMSnjhMWQTGEzA1eUGqxLVva.","param":"input","type":"invalid_request_error"}}`)
	resp := &http.Response{
	...
	assert.Equal(t, "incomplete_tool_call_state", errField["code"])
	assert.Equal(t, "input", errField["param"])
	assert.Contains(t, errField["message"], "Start a new task")
	assert.NotContains(t, rec.Body.String(), "fcbMSnjhMWQTGEzA1eUGqxLVva")
}

func TestIsMissingToolOutputError(t *testing.T) {
	assert.True(t, isMissingToolOutputError(
		http.StatusBadRequest,
		"No tool output found for tool search call call_123.",
	))
	assert.False(t, isMissingToolOutputError(
		http.StatusBadGateway,
		"No tool output found for tool search call call_123.",
	))
	assert.False(t, isMissingToolOutputError(
		http.StatusBadRequest,
		"Invalid schema for field input",
	))
}
