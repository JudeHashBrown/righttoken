...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	referralRuleRepository := repository.NewReferralRuleRepository(client)
	referralCommissionRepository := repository.NewReferralCommissionRepository(client, db)
	referralService := service.NewReferralService(userRepository, referralCommissionRepository, referralRuleRepository, settingService)
	...
	recallSSOService := service.NewRecallSSOService(configConfig)
	recallVisitService := service.NewRecallVisitService(configConfig)
	...
	referralHandler := handler.NewReferralHandler(referralService, userService)
	adminReferralHandler := admin.NewReferralAdminHandler(referralService)
	...
	openAIGatewayService.SetReferralService(referralService)
	...
	recallSSOHandler := handler.NewRecallSSOHandler(userService, recallSSOService)
	analyticsHandler := handler.NewAnalyticsHandler(recallVisitService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, referralHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, recallSSOHandler, analyticsHandler, idempotencyCoordinator, idempotencyCleanupService, service.ReferralWiring{})
