...
- 公共邮件模板、可视化与 HTML 源码编辑、正文图片、图片附件和发送预览
- 企微群机器人脱敏通知、失败重试和死信留痕
- RightToken 实时事件接口、模拟数据源和定时全量校准

## 客户负责人分配

- 用户首次同步时，系统根据注册归属地自动分配负责人；省份或州级规则优先于国家规则。
- 每位运营的负责国家和地区可在“成员与权限”页面直接配置，保存后系统自动重算仍处于自动分配状态的客户。
- 没有匹配到地区规则时，由主管理员自动暂管，不会产生没有负责人的客户。
- 主管理员和管理员可在用户列表或详情页手动调整负责人，未完成任务会同步转给新负责人。
- 人工调整后，后续数据同步、地区变化和规则重算都不会覆盖该负责人；点击“恢复自动分配”后才重新采用最新规则。
- 主管理员和管理员也可在用户详情中“确认所属地区”。人工确认的地区不会被邮箱、注册 IP 或主站同步覆盖；点击“恢复自动判定”后才重新采用最新来源。
- 人工地区与人工负责人相互独立：更改或恢复地区不会覆盖人工负责人，更改或恢复负责人也不会解除人工地区。
- 撤销成员权限前必须选择一名启用成员作为接管人。系统在同一事务中停用
  原账号，把其全部客户和未完成任务交给接管人，并将客户负责人锁定为人工
  指定；已完成和已取消任务继续保留原负责人历史。
...
使用仓库中的生产 Compose 时，上述变量对应
`RECALL_MAIL_ASSET_*`。部署解析阶段会强制检查存储类型、桶、区域和
访问凭据；缺失时容器不会带着不可用配置启动。对象存储连接或写入失败时，
上传接口返回“图片存储暂不可用，请联系管理员”，且不会创建孤立数据库记录。
...
新邮件、任务邮件、回复和公共模板统一支持“可视化”“HTML 源码”
和“发送预览”三种模式。HTML 源码可直接粘贴或导入完整 `.html` 文件，
支持 `DOCTYPE`、`html/head/body`、内联样式、`style`、响应式
`@media` 和表格布局。

发送预览会使用服务端最终清洗结果，并同步生成纯文本版本。脚本、事件属性、
危险链接和不安全样式会被移除；HTTPS 外链图片会保留并提示隐私与兼容性风险，
上传图片则以内嵌图片发送。复杂 HTML 切回可视化模式前会提示可能发生的格式
简化，避免误改完整模板。
...
- 运营数据写入独立的 `recall` schema；主站业务表只读，不会由运营后台修改。
