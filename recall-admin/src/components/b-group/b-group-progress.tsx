...
export type BGroupPanel = "mail" | "contact" | "coupon" | "maintenance";

export function BGroupProgress({ user, active, onSelect }: {
  user: BGroupSelectedUser;
  active: BGroupPanel | null;
  ...
  const steps = [
    ...
    { key: "contact" as const, title: "登记联系方式", detail: "微信 · TG · 手机号", complete: user.progress.contactComplete },
    { key: "coupon" as const, title: "送优惠券", detail: user.coupon?.status === "SUCCEEDED" ? "USD 1.43 · 已送" : "USD 1.43 · 尚未赠送", complete: user.progress.couponComplete },
  ...
  return <div className={styles.progress}>
    <div className={`${styles.step} ${styles.identity}`}><strong>#{user.registrationSequence}</strong><small>{user.email}</small></div>
    {steps.map((step) => <div className={styles.stepPair} key={step.key}>
