...
export function DGroupMaintenancePanel({ user }: { user: DGroupSelectedUser }) {
  ...
  return (
    <section className={styles.panel}>
      ...
      <div className={styles.records}>{user.maintenanceRecords.map((record) => (
        <div className={styles.record} key={record.id}><time>{new Intl.DateTimeFormat("zh-CN", { timeZone: "Asia/Shanghai", year: "numeric", month: "2-digit", day: "2-digit" }).format(record.occurredAt)}</time><p>{record.body}{record.effective ? "" : "（已退信，不计入有效维护）"}</p></div>
      ))}</div>
    </section>
  );
}
